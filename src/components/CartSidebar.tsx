      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {/* Cart items — rendered via map(), quantity from state */}
                  ...
                  cart.map((item) => (
                    ...
                    >
                      <img
                        src={item.image}
                        alt={item.name}
                        className="w-14 h-14 rounded-xl object-cover bg-orange-100 shrink-0"
                      />
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-sm text-foreground truncate">{item.name}</p>
                        {/* Subtotal = price × quantity, computed dynamically */}
